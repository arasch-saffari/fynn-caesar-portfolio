...
import { Target, Gamepad2, Heart, Unlock, Loader2 } from 'lucide-react';
...
const IntroScreen: React.FC<IntroScreenProps> = ({ onStart }) => {
  ...
  // Wait for Fonts to load before showing Start button
  // This prevents the Canvas from rendering text with a fallback font initially
  useEffect(() => {
    ...
    <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black overflow-hidden h-[100dvh]">
      ...
      <div className="absolute inset-0 flex flex-col justify-center items-center select-none pointer-events-none overflow-hidden">
         ...
         <div className="w-[120%] -rotate-[6deg] flex flex-col gap-4 md:gap-8 origin-center">
            <PatternRow Icon={PixelSmiley} color="text-yellow-400" direction="left" speed="duration-[60s]" opacity="opacity-30" />
            <PatternRow Icon={PixelHeart} color="text-pink-500" direction="right" speed="duration-[100s]" opacity="opacity-40" />
            <PatternRow Icon={PixelEye} color="text-purple-400" direction="left" speed="duration-[80s]" opacity="opacity-30" />
         </div>
      </div>
      ...
      <div className="relative z-10 flex flex-col items-center gap-6 md:gap-10 p-4 w-full max-w-5xl h-full justify-center">
        ...
        <div className="h-20 flex items-center justify-center">
            {isReady ? (
                ...
                <div className="flex flex-col items-center gap-2 text-pink-500/70 animate-pulse">
                     <Loader2 className="w-8 h-8 animate-spin" />
                     <span className="font-pixel text-xs tracking-widest">LOADING ASSETS...</span>
                </div>
            )}
        </div>
      </div>
